...
def write_OHH_coor(file, 
  ...
  # OH
  OT_offset = np.array([A[0], A[1], H2O_height])
  ...
  O_coor = O_base + OT_offset
  H1_coor = H1_base + OT_offset
  
  # H
  if sym_point == 'OT':
    H2_coor = np.array([A[0], A[1], H2O_height]) + a2
  elif sym_point == 'fcc':
    H2_coor = np.array([B[0], B[1], H2O_height])
  elif sym_point == 'hcp':
    H1_coor = H2_base + OT_offset
    H2_coor = np.array([C[0], C[1], H2O_height])
  else:
