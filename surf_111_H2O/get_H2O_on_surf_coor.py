...
def write_H2O_coor(file, 
  ...
  H2O_height = A[2] + height_from_surf
  ...
  if sym_point == 'OT':
    offset = np.array([A[0], A[1], H2O_height])
  elif sym_point == 'fcc':
    offset = np.array([B[0], B[1], H2O_height])
  elif sym_point == 'hcp':
    offset = np.array([C[0], C[1], H2O_height])
  else:
...
with open(f'H2O_at_{sym_point}-coor.dat', 'w') as file:
  write_H2O_coor(file, relaxed_coor_list, sym_point=sym_point)
