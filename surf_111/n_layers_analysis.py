...
surf_energy = 0.5 / A * (energy - n_layers * ebulk) * 16.0217663 * 1000 # surface energy in mJ/m**2
...
ax.set_ylabel(r'$\Delta E$ (mJ/$m^2$)', fontdict=base_font)
ax.grid()
plt.savefig(f'../../report/Figures/surf_energy.png')
plt.show()

_, ax = plt.subplots(1, 1, figsize=(11, 8))
ax.plot(n_layers, surf_energy, 'o', color='royalblue')
ax.set_title(r'Surface energy with increasing number of layers', fontdict=title_font)
ax.set_xlabel('n_layers', fontdict=base_font)
ax.set_ylabel(r'E (mJ/$m^2$)', fontdict=base_font)
ax.grid()
plt.show()
