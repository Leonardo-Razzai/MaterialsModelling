...
def write_CO2_coor(file, 
  ...
  if sym_point == 'OT':
    offset = np.array([A[0], A[1], CO2_height])
  elif sym_point == 'fcc':
    offset = np.array([B[0], B[1], CO2_height])
  elif sym_point == 'hcp':
    offset = np.array([C[0], C[1], CO2_height])
  else:
...
with open(f'CO2_at_{sym_point}-coor.dat', 'w') as file:
  write_CO2_coor(file, relaxed_coor_list, sym_point=sym_point)
