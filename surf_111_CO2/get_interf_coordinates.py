...
def split_slabs_coor(relaxed_coor_list: list):
  ...
  for coor_dict in relaxed_coor_list:
    if coor_dict['index'] < num_layers * num_atoms_per_layer:
      ...
      CO2.append(coor_dict)
    else:
      slab_down.append(coor_dict)
  
  return slab_up, CO2, slab_down
...
def write_next_coor(file, 
  ...
  for coor_dict in CO2:
    if coor_dict['element'] == 'C':
...
def write_CO2_coor(file, 
  ...
  CO2_height = OT[2] + dist_between_interf / 2
  
  C_base = np.array([0, 0, 0])
  O1_base = np.array([1, 0, 0]) * bond_length
  O2_base = np.array([-1, 0, 0]) * bond_length
  
  offset = np.array([OT[0], OT[1], CO2_height]) + a1 + a2
  
  C_coor = C_base + offset
  O1_coor = O1_base + offset
  O2_coor = O2_base + offset
  
  print_atom_coor(file, atom='O', coor=O1_coor)
  print_atom_coor(file, atom='O', coor=O2_coor)
  
  line = print_atom_coor(file, atom='C', coor=C_coor, write=False)
  file.write(line + '  1  1  0\n')
...
if __name__ == "__main__":
  ...
  try:
    ...
    slab_up, CO2, slab_down = split_slabs_coor(relaxed_coor_list)
    
    with open(f'interf_at_{dist_between_interf}-coor.dat', 'w') as file:
      write_next_coor(file, slab_up, CO2, slab_down)
    ...
    with open(f'interf_at_{dist_between_interf}-coor.dat', 'w') as file:
      ...
      write_CO2_coor(file, relaxed_coor_list)
