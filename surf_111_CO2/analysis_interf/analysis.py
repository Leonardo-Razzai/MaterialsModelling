...
class change_dist:
  def __init__(self) -> None:
    self.data = pd.read_csv(f'data/data_interf.dat', sep=' ')
  ...
  def plot_data(self):
    ...
    ax.plot(d, f_inter(d), '--', color='black', lw=2.5)
    ax.plot(self.dist, self.e_tot - self.far_energy, 'o', color='red', markersize=8)
    ...
    ax.grid()
    plt.savefig(f'../../../report/Figures/CO2/change_distance_interf.png')
    plt.show()
